...
class CreditsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			final json:Array<CreditsSection> = Json.parse(Assets.getText(creditsFile));
			for(num => section in json)
			{
				creditsStuff.push([section.header]);
				for(credit in section.credits)
					creditsStuff.push([credit.name, credit.icon, credit.description, credit.link, credit.color]);
				if(num != json.length - 1) creditsStuff.push(['']);
			}
		}
		else // just to be safe
		{
			final defaultList:Array<Array<String>> = [
				["Psych Engine Team"],
	...
	override function update(elapsed:Float)
	{
		...
		for(item in grpOptions.members)
		{
			if(!item.bold)
			{
				final lerpVal:Float = Math.exp(-elapsed * 12);
				if(item.targetY == 0)
				{
	...
	{
		// first we check the new system
		var creditsFile:String = Paths.mods('$folder/data/credits.json');
		#if TRANSLATIONS_ALLOWED
		var translatedCredits:String = Paths.mods('$folder/data/credits-${ClientPrefs.data.language}.json');
		if(FileSystem.exists(translatedCredits)) creditsFile = translatedCredits;
		#end
		if(FileSystem.exists(creditsFile))
		{
			final json:Array<CreditsSection> = Json.parse(File.getContent(creditsFile));
			for(section in json)
			{
				creditsStuff.push([section.header]);
				for(credit in section.credits)
					creditsStuff.push([credit.name, credit.icon, credit.description, credit.link, credit.color, folder]);
				creditsStuff.push(['']);
			}
			return;
		}

		// then we check the old system
		creditsFile = Paths.mods('$folder/data/credits.txt');
		#if TRANSLATIONS_ALLOWED
		translatedCredits = Paths.mods('$folder/data/credits-${ClientPrefs.data.language}.txt');
		if(FileSystem.exists(translatedCredits)) creditsFile = translatedCredits;
		#end
		if(FileSystem.exists(creditsFile))
		{
			final firstarray:Array<String> = File.getContent(creditsFile).split('\n');
			for(i in firstarray)
			{
