...
class ABotSpeaker extends FlxSpriteGroup
{
	...
	public function new(x:Float = 0, y:Float = 0)
	{
		...
		for (i in 1...VIZ_MAX+1)
		{
			...
			viz.frames = Paths.getSparrowAtlas('abot/aBotViz');
		...
		eyes.frames = Paths.getAnimateAtlas('abot/systemEyes');
		eyes.anim.addBySymbolIndices('lookleft', 'a bot eyes lookin', [for (i in 0...18) i], 24, false);
		eyes.anim.addBySymbolIndices('lookright', 'a bot eyes lookin', [for (i in 18...36) i], 24, false);
		eyes.anim.play('lookright', true);
		eyes.anim.curAnim.finish();
		add(eyes);

		speaker = new FlxAnimate(-65, -10);
		speaker.frames = Paths.getAnimateAtlas('abot/abotSystem');
		speaker.anim.addBySymbol('anim', 'Abot System', 24, false);
		speaker.anim.play('anim', true);
		speaker.anim.curAnim.finish();
