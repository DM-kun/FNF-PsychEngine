...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			if(weekIsLocked(week)) continue;

			final leWeek:WeekData = WeekData.weeksLoaded.get(week);
		...
		player = new MusicPlayer(this, this.controls, [scoreBG, diffText, scoreText]);
		add(player);
	...
	override function update(elapsed:Float)
	{
				...
				if(PlayState.SONG.needsVoices)
				{
					...
					try
					{
						...
						if(loadedVocals != null && loadedVocals.length > 0)
						{
							...
							player.voices.push(opponentVocals);
							//trace('yaaay!!');
						}
		...
		if(!player.playingMusic)
		{
			...
			{
				persistentUpdate = false;
				final weekFile:String = getPath('weeks/' + WeekData.getWeekFileName() + '.json', TEXT, null, true);
				MusicBeatState.switchState(new states.editors.WeekEditorState.WeekEditorFreeplayState(WeekData.getWeekFile(weekFile)));
			}
			#end*/
		}
	...
	public function destroyFreeplayVocals()
	{
		...
		{
			if(voc != null) vocals.stop();
			voc = FlxDestroyUtil.destroy(voc);
		}
		player.voices = [];
	}
	...
	function changeSelection(change:Int = 0, playSound:Bool = true)
	{
		...
		{
			final icon:HealthIcon = iconArray[num];
			item.alpha = (item.targetY == curSelected) ? 1 : 0.6;
			icon.alpha = (item.targetY == curSelected) ? 1 : 0.6;
		}
		...
		player.songName = songs[curSelected].songName;
